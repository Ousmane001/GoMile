...
export class GetApiKeyResponseDto {
  ...
  @ApiProperty({ type: 'string', format: 'date-time', nullable: true, example: null, description: 'Set when the key has been revoked' })
  revokedAt: Date | null;

  @ApiProperty({ type: 'string', format: 'date-time', nullable: true, example: null, description: 'Expiration date, null means no expiration' })
  expiresAt: Date | null;

  @ApiProperty({ type: () => ApiKeyStoreDto, description: 'Store this API key is scoped to' })
