...
export class DriverMeController {
  ...
  @ApiOperation({
    summary: 'Get available missions nearby',
    description:
      "Returns orders in SEARCHING_DRIVER status within the driver's delivery radius using PostGIS.",
  })
  ...
  @ApiConflictResponse({
    description: 'Mission was already accepted by another driver',
  })
  ...
  @ApiOperation({
    summary: 'Get pickup code (handshake A)',
    description:
      'Returns the type A pickup code the driver must show to the merchant. Only available while order is in DRIVER_ACCEPTED status.',
  })
  @ApiOkResponse({
    schema: {
      properties: { pickupCode: { type: 'string', example: '048291' } },
    },
  })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  @ApiNotFoundResponse({ description: 'Mission not found' })
  ...
  @ApiOperation({
    summary: 'Retrieve OTP pickup code for a mission',
    description:
      'Allows the driver to retrieve the handshake type A pickup code in case the app was closed. Only available while the mission is in DRIVER_ACCEPTED status.',
  })
  @ApiOkResponse({
  ...
  @ApiNotFoundResponse({ description: 'Mission not found' })
  @ApiInternalServerErrorResponse({
    description: 'Handshake not found, contact backend developer',
  })
  ...
  @ApiOperation({
    summary: 'Pick up a mission (handshake A — merchant code)',
    description:
      'Merchant provides driver a code, driver must enter it to validate pick up. 3 attempts allowed.',
  })
  @ApiOkResponse({
    description: 'Mission picked up successfully',
    schema: {
  ...
  @ApiNotFoundResponse({ description: 'Mission not found' })
  @ApiConflictResponse({
    description: 'Mission already picked up or cancelled',
  })
  @ApiInternalServerErrorResponse({ description: 'Bad order state' })
  @ApiTooManyRequestsResponse({
    description: 'Too many incorrect code attempts (3 allowed)',
  })
  ...
  @ApiOperation({
    summary: 'Deliver a mission (handshake B — customer code)',
    description:
      'Customer receives a code, driver must enter it to validate delivery.',
  })
  @ApiOkResponse({
  ...
  @ApiNotFoundResponse({ description: 'Mission not found' })
  @ApiConflictResponse({
    description: 'Mission already delivered or cancelled',
  })
  @ApiInternalServerErrorResponse({ description: 'Bad order state' })
  @ApiTooManyRequestsResponse({
    description: 'Too many incorrect code attempts (3 allowed)',
  ...
  @ApiOperation({
    summary: 'Update driver position',
    description:
      'Syncs driver lat/lng and PostGIS location. Should be called frequently while driver is online.',
  })
  @ApiOkResponse({
  ...
  async updateLocation(
    ...
    return this.driverMeService.updateDriverPosition(
      user,
      dto.latitude,
      dto.longitude,
    );
  ...
  @ApiOperation({
    ...
    description:
      'If the driver was available, update the driver status to offline and vice versa. A driver with an ongoing order (busy) cannot change its status.',
  })
  @ApiOkResponse({
    description: 'Current status of the driver after the toggle',
    schema: { properties: { message: { type: 'string' } } },
  })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  @ApiConflictResponse({
    description: 'Driver with ongoing orders cannot change its status',
  })
  @Patch('availability')
  @HttpCode(HttpStatus.OK)
  async toggleDriverStatus(@CurrentUser() user: AuthenticatedUser) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns missions currently in DRIVER_ACCEPTED or PICKED_UP status.',
  })
  ...
  @ApiOperation({
    summary: 'Get missions history',
    description: 'Returns missions in DELIVERED or CANCELLED status.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Changing address fields will reset KYC status to NOT_SUBMITTED.',
  })
  ...
  @ApiOkResponse({
    schema: {
      properties: {
        activeVehicle: { type: 'string', enum: Object.values(VehicleType) },
      },
    },
  })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  ...
  @ApiOperation({
    ...
    description:
      "Returns the driver's current KYC status, uploaded documents, and latest submission.",
  })
  ...
  @ApiOperation({
    summary: 'Submit for KYC review',
    description:
      'Triggers a KYC review request. Driver must have uploaded at least one document first. Sets kycStatus to PENDING.',
  })
  @ApiOkResponse({ schema: { properties: { message: { type: 'string' } } } })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  @ApiNotFoundResponse({ description: 'Driver not found' })
  @ApiConflictResponse({
    description:
      'A KYC submission is already pending, or no documents uploaded yet',
  })
  @Post('kyc')
  @HttpCode(HttpStatus.OK)
  ...
  @ApiOperation({
    summary: 'Retrieve driver dashboard',
    description:
      'Driver dashboard contains the primary informations of a driver of the day, including its current status, total earnings, last known location as well as its coverage radius',
  })
  ...
  @ApiOperation({
    summary: 'Get wallet',
    description:
      "Returns the driver's wallet balance, currency and pending amount.",
  })
  @ApiOkResponse({
    schema: {
      properties: {
  ...
  @ApiOperation({
    summary: 'Get wallet entries',
    description: "Returns the driver's wallet transaction history.",
  })
  @ApiOkResponse({
    schema: {
      type: 'array',
  ...
  @ApiOperation({
    ...
    description:
      'Driver requests a withdrawal from their wallet. Amount must be positive and not exceed available balance.',
  })
  @ApiOkResponse({ schema: { properties: { message: { type: 'string' } } } })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  @ApiNotFoundResponse({ description: 'Driver not found' })
  ...
  @ApiOperation({
    ...
    description:
      'Returns all typed documents (license, CNI, RIB, etc.) uploaded by the driver.',
  })
  @ApiOkResponse({
    schema: {
      type: 'array',
  ...
  @ApiOperation({
    ...
    description:
      'Stores a typed document (license, CNI, RIB, etc.). Get the URL first via POST /uploads/presign.',
  })
  @ApiOkResponse({
    schema: {
      properties: {
  ...
  @ApiOperation({
    ...
    description: 'Deletes the document record and removes the file from S3.',
  })
  @ApiOkResponse({ schema: { properties: { message: { type: 'string' } } } })
  @ApiUnauthorizedResponse({ description: 'Invalid or missing JWT' })
  @ApiNotFoundResponse({
    description: 'Document not found or does not belong to driver',
  })
